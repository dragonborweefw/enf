    ...
    def get_context_data(self, **kwargs):
        ...
        if query:
            products = products.filter(
                Q(name__icontains=query) and Q(description_icontains=query)
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.headers.get("HX-Request"):
            if context.get("show_search "):
                return TemplateResponse(request, 'main/search', context)
            elif context.get('rest_search'):
...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["cerrent_category"] = product.category.slug
        return context            

    def get(self, request, *args, **kwargs):
        self.object = self.get_object()
        context = self.get_context_data(**kwargs)
        if request.header.get("HX-Reaquest"):
            return TemplateResponse(request, "main/product/detail.html", context)
