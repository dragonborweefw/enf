# from decimal import Decimal
# from typing import Iterator, Any
# from django.http import HttpRequest
# from django.contrib.sessions.backends.base import SessionBase
# from main.models import Product


# class Cart:
#     def __init__(self, request: HttpRequest) -> None:
#         self.session: SessionBase = request.session
#         cart: dict[str, Any] | None = self.session.get('cart')
#         if not cart:
#             cart = self.session['cart'] = {}
#         self.cart: dict[str, Any] = cart

#     def add(
#         self,
#         product: Product,
#         size: str,
#         quantity: int = 1,
#         override_quantity: bool = False,
#     ) -> None:
#         product_id: str = str(product.id)
#         size_name: str = str(size)
#         cart_key: str = f"{product_id}_{size_name}"

#         if cart_key not in self.cart:
#             self.cart[cart_key] = {
#                 'quantity': 0,
#                 'price': str(product.price),
#                 'product_id': product_id,
#                 'size': size_name,
#             }

#         if override_quantity:
#             self.cart[cart_key]['quantity'] = quantity
#         else:
#             self.cart[cart_key]['quantity'] += quantity

#         self.save()

#     def save(self) -> None:
#         self.session.modified = True

#     def remove(self, product: Product, size: str) -> None:
#         product_id: str = str(product.id)
#         size_name: str = str(size)
#         cart_key: str = f"{product_id}_{size_name}"

#         if cart_key in self.cart:
#             del self.cart[cart_key]
#             self.save()

#     def update_quantity(self, product: Product, size: str, quantity: int) -> None:
#         if quantity <= 0:
#             self.remove(product, size)
#         else:
#             self.add(product, size, quantity, override_quantity=True)

#     def __iter__(self) -> Iterator[dict[str, Any]]:
#         product_ids: list[str] = [item['product_id'] for item in self.cart.values()]
#         products = Product.objects.filter(id__in=product_ids)
#         cart: dict[str, Any] = self.cart.copy()

#         for product in products:
#             for cart_key, cart_item in cart.items():
#                 if cart_item['product_id'] == str(product.id):
#                     cart_item['product'] = product
#                     cart_item['total_price'] = (
#                         Decimal(cart_item['price']) * cart_item['quantity']
#                     )
#                     yield cart_item

#     def __len__(self) -> int:
#         length: int = sum(item['quantity'] for item in self.cart.values())
#         return length

#     def get_total_price(self) -> Decimal:
#         total: Decimal = sum(
#             Decimal(item['price']) * item['quantity']
#             for item in self.cart.values()
#         )
#         return total

#     def clear(self) -> None:
#         del self.session['cart']
#         self.save()

#     def get_cart_items(self) -> list[dict[str, Any]]:
#         items: list[dict[str, Any]] = []
#         for item in self:
#             items.append({
#                 'product': item['product'],
#                 'quantity': item['quantity'],
#                 'size': item['size'],
#                 'price': Decimal(item['price']),
#                 'total_price': item['total_price'],
#                 'cart_key': f"{item['product_id']}_{item['size']}"
#             })
#         return items